...
def diferenciacion(x, y, ind, fin, h):
    print("\n")
    print("*****TABLA*****\n")
    for i in range(ind, fin+1):
        if i == ind:
            print(f"x{i}:{x[i]} - y{i}:{y[i]} - f'(x): -")
        else:
            try:
                print(f"x{i}:{x[i]} - y{i}:{y[i]} - f'(x): {(y[i+1]-y[i-1])/(2*h)}")
    ...
    while dif_:
        ...
        if opcion == 1:
            ...
            if len(x) == 0:
                ...
                continue
            mostrar(x, y)
            ind = int(input("Dame el indice del valor inicial: "))
            fin = int(input("Dame el indice del valor final: "))
            if (fin-ind)+1 < 3:
                print("Necesitas al menos 3 registros.")
                sleep(1.5)
                continue
            diferenciacion(x, y, ind, fin, h)
